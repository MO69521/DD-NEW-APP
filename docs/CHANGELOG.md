...
### 新增
- 启动页底部加入「点点 · 穿书新世界」透明品牌 Logo，按参考图占屏宽 68.5%（最大 315px）展示，并避让系统底部安全区。
...
## 2026-07-20（浅黄星尘余额增加可兑换提示）

### 修改
- `yellow_light` 的星尘余额数字上方增加红色「可兑换能量」标签，底部尖角居中指向数字。
- 标签复用 `hotSaleBadge` 红底与 `welfareHotSaleBadge` 恒白文字；`yellow_dark` / `pink_light` 不显示。

### 影响模块
- 福利页与我的页复用的虚拟货币余额栏；设计系统说明同步。

### Breaking Changes
- 无。

## 2026-07-20（浅黄余额栏去白底并两端对齐）

### 修改
- `yellow_light` 的 `CurrencyBalanceBar` 移除白色填充、描边、圆角模糊和外层内边距。
- 四项余额中首项左对齐、末项右对齐，中间两项保持均匀分布；`yellow_dark` / `pink_light` 视觉不变。
...
## 2026-07-20（浅色主题顶部与底部 Chrome 改为不透明实底）

### 修改
- `pink_light` / `yellow_light` 的 `AppBlurredChromeBar` 统一跳过背景模糊，背景启用时使用 100% 不透明纯白。
- 浅色底部导航 `navBarBackground` 收敛为 `white100`；`yellow_dark` 保持原毛玻璃与深色底。
- 福利页未滚动重叠时仍展示浅黄彩头，吸顶覆盖内容后使用纯白实底。

### 影响模块
- 全站顶部 Chrome、吸顶标题栏、底部导航与固定底部操作栏；设计系统说明同步。

### Breaking Changes
- 浅色主题不再展示顶部/底部毛玻璃透底效果。

## 2026-07-20（标题栏右侧图标移除背景填充）

### 修改
- `AppTopBarIconButton` 默认改为裸图标样式：不绘制背景填充或圆框，保留统一点击热区，三个主题同步生效。
- `AppTopBar` 左侧返回按钮显式使用 `showFrame: true`，保留原圆形磨砂框。

### 影响模块
- 书城、福利、榜单、伙伴、我的等标题栏右侧图标动作；设计系统说明同步。

### Breaking Changes
- `AppTopBarIconButton` 默认视觉由圆形磨砂框改为裸图标；需要旧样式的调用点传 `showFrame: true`。

## 2026-07-20（浅黄彩头仅保留福利页）

### 修改
- 用户指定 `yellow_light` 顶部黄色渐隐装饰仅在福利页显示；书城首页与书架移除 `AppTabTopTexture`。
- `yellow_dark` / `pink_light` 视觉保持不变。

### 影响模块
- 书城首页、福利页、书架页顶部装饰；设计系统说明同步。
