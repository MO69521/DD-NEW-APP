...
| 类别 | 真源 | 内容 |
|---|---|---|
| 颜色原色 | [app_palette.dart](../lib/core/theme/app_palette.dart) | **全项目唯一**允许 `Color(0x…)`；白/黑透明度阶（`whiteAlpha*`/`blackAlpha*`）、中性色阶（`neutralCool*`）、彩色原色（`pink*`/`orange*`/`yellow500`…）、主题基色 alpha 阶 |
...
| 类别 | 真源 | 示例 |
|---|---|---|
| 主题壳源色 | [app_brand_colors.dart](../lib/core/theme/app_brand_colors.dart) | `backgroundDark`、`accent`、`dialogBackground`、`bgTint*`（按 `themeId` 选原色） |
...
深浅翻转：`AppColors` 内 `_isLight` 三元根据主题实验包切换语义色取值，语义名恒定、调用点零改动。
