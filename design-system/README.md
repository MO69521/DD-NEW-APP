...
| 浅色实体 | `neutralBlue950 #1A1A2E` · `neutralCool600 #6B7280` · `neutralGray400 #9B9B9B` · `neutralCool100 #F3F4F6` · `neutralCool200 #E5E7EB` · `pink100 #F4D9E4` · `neutralCool50 #F8F7FC` |
...
| 主强调 | `yellow500 #FFE847`（+ `yellow500Alpha04`/`yellow500Alpha08`）· `pink500 #FF4D88`（+ `pink500Alpha04`/`pink500Alpha08`/`pink500Alpha40`） |
...
> 上层映射：`AppBrandColors.partnerPrimary = pink500`、`primary = pink_light ? pink500 : yellow500`、`textPrimary = pink_light ? neutralBlue950 : neutralWhite`、`surface = pink_light ? neutralWhite : neutralCool900` … 语义名与调用点全部不变，仅底层改为引用 `AppPalette`。
>
...
| `blackAlpha60` | 60% | `0x99000000` | 封面选择遮罩；榜单名次角标底（语义别名 `rankingMutedBadgeScrim`，第 4 名起，20px，仅右下角 `md` 圆角，保证白色数字清晰）|
...
| 语义 token | dark 解析 | pink_light 解析 | 职责 |
|------|------|------|------|
| `primary` | `#FFE847` | `#FF4D88` | 主强调 |
| `onPrimary` | `#090E17` | `#FFFFFF` | 主强调色面上的文字 / 图标 |
| `background` | `#090E17` | `#F4F2F4` | 页面背景 |
| `surface` | `#151B24` | `#FFFFFF` | 普通容器面 |
| `surfaceSoft` | `#111722` | `#F8F7FC` | 弱容器面 |
| `surfaceElevated` | `#131820` | `#FFFFFF` | 浮层 / 弹窗面 |
| `textPrimary` | `#FFFFFF` | `#1A1A2E` | 一级文字 |
| `textSecondary` | `#9AA0AA` | `#6B7280` | 二级文字 |
| `textTertiary` | `#737B86` | `#9B9B9B` | 三级文字 / 占位 |
| `borderSubtle` | `whiteAlpha04`（4% 白 `0x0AFFFFFF`） | `#F4D9E4` | 弱描边（深色态全局边框，随底自然融合） |
| `divider` | `#232A33` | `#F3F4F6` | 分割线 |
| `overlayScrim80` | `blackAlpha80` | `blackAlpha80` | 弹窗遮罩 |
...
语义引用：`chromeBarScrim = bgTint80`，`bottomNavScrim`（深 `bgTint90` / 浅 `white100` 纯白），`topChromeBarScrolledScrim`（深 `bgTint80` / 浅 `white100` 纯白），用于压实全局毛玻璃、减少下方内容透出。浅色态底部导航与 `AppPageChrome` 顶栏改为纯白实心底；书详情顶栏走 `chromeBarScrim`，保持磨砂不填白。
...
| `pink_light` | `#F4F2F4`（浅灰粉，新增） | `#FF4D88`（玫粉，复用 `partnerPrimary`） | `#F4F2F4`（同基色标准透明度阶） | dialog/浮层 `#FFFFFF`；壳文字深墨 `#1A1A2E` | 粉色浅色系；需 §4.2.4 中性翻转层 |
...
- **primary 上的文字/图标**：核心语义 token `onPrimary`（深色态深墨 `#090E17`；浅色态白 `#FFFFFF`）。`onAccent` 为兼容别名。
- **primary 派生填充**：`primarySoft`（4%）与 `segmentedSelectedFill`（8%）深色态为品牌黄叠加，浅色态随主强调翻为玫粉叠加（`0x0AFF4D88` / `0x14FF4D88`）。
- **选中白胶囊**：`discussionFilterSelectedBackground` 深色态白 `#FFFFFF`，浅色态翻为 primary（玫粉）胶囊 + `onPrimary` 白字。
- **底部导航**：选中态图标随 `primary` 运行时着色（深色黄 / 浅色粉；图标 SVG 为**单色黑**源，`srcIn` 着色）+ 标签 `primary`（自动随主强调翻粉）+ 未选中图标/标签中性灰；`aurora` 背景默认走 `primary`。
...
> 新增色值：`#F4F2F4`（背景）、`#F4D9E4`（浅粉描边）、`#FF4D88` 低透明派生（`0x0AFF4D88` / `0x14FF4D88`）；其余复用既有源色。翻转开关/源值命名前缀：`_isLight` / `_ink*` / `_light*` / `_dark*` / `onPrimary`。v1 已知未覆盖（后续视觉走查）：极光片元着色器如需浅色专属配色、少量 `whiteAlphaNN` 渐变遮罩/CTA 扫光（本为图/CTA 上光效，保留白色）。
...
> **福利页浅色翻转（`app_welfare_colors.dart`）**：福利页签到 / 任务区原先把 `whiteNN`（白透明叠加）与个别深色 `Color(0x…)` 直接用作**面 / 描边 / 文字 / 里程碑圆点**，浅色实验包（`pink_light`）下不翻转会「白底看不见、深点变黑块、白字消失」。已按 `_isLight`（`AppBrandColors.isLightExperiment`）分支：**深色分支保持原值不变**，浅色分支翻到已有实体语义 token —— 面 `checkInDayBg/…Header/rechargePriceBg/taskActionBg/taskRewardChipBg/taskTimelineBubbleReached/checkInProgressTrack/taskTimelineTrack → surfaceSoft`（轨道/分割类 → `divider`）、描边 `checkInDayBorder → borderSubtle`、圆点 `checkInProgressDot*/taskTimelineDot* → border/divider`、文字 `checkInDayLabelMuted/checkInMilestoneLabel/taskRewardChipMutedText/taskProgressLabel → textSecondary`、`checkInMilestoneAmount/taskRewardChipText → textPrimary`。未引入新色值。
>
...
| `guessLikeCardBackground` | `surfaceSoft`（深 `neutralCool920` / 浅 `neutralCool50`） | 猜你喜欢卡片底 |
| `guessLikeTagBackground` | `surface`（深 `neutralCool900` / 浅 `neutralWhite`） | 猜你喜欢标签底 |
| `guessLikeTagBorder` | `borderSubtle`（深 `whiteAlpha04` / 浅 `pink100`） | 猜你喜欢标签描边 |
...
**例外（保持恒暗，加行内注释 `// light-audit: keep-dark` 或 `// light-audit: effect`）**：压在封面/头图/角色立绘上的遮罩·描边·阴影、彩色角标上的恒白字、alpha 淡出蒙版、扫光高光、品牌恒定色（金/橙/粉紫 tint）、伙伴沉浸式互动场景。
...
| 底部导航 | 底 `navBarBackground` · 选中 `primary` 着色图标 + 标签（深黄/浅粉）· 未选 `iconMutedSecondary` |
...
- 列表行 / 可点击条目行尾指示统一用**实心三角箭头** `assets/icons/arrow_right.svg`（`AppIcon`，`AppSpacing.sm`=12，色 `textTertiary`）；禁止用 Material `chevron_right` 等描边箭头（设置 / 帮助与反馈 / 通知等已统一）。
