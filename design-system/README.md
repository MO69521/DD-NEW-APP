...
| 书架页 | 顶栏 / 阅读横幅 / 空状态 / 封面角标 | `bookshelf*` · `bookCoverTag*`（含 `bookCoverTagBlurSigma`） |
...
| `BookCoverTagBadge` | `shared/components/book_cover_tag_badge.dart` | 封面角标：全主题统一 `bookCoverTagBlurSigma` 磨砂底；更新=主色底+onPrimary 字；完结/连载=黑 60% 底+恒白字+黑 4% hairline 描边 |
