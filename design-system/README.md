...
- **区块尾随** `sectionTrailingAction`：`md` / regular / `textOnDarkMuted`，用于 `SectionTrailingAction`（目录状态、「滑动查看更多」、`SectionHeader`「更多」）；`bookDetailCatalogMeta` / `bookDetailSectionHint` 为其别名。
...
| 透明效果原语 | `whiteAlpha100…whiteAlpha00` / `blackAlpha100…blackAlpha00`（主要用于遮罩、蒙版、扫光、渐隐等 effect；不作为组件底色 / 文本色库。**例外**：深色态弱描边 `borderSubtle` 取 `whiteAlpha04`（4% 白），见 §4.2） |
...
> 上层映射：`AppBrandColors.partnerPrimary = pink500`、`primary = pink_light ? pink500 : yellow500`（`yellow_dark`/`yellow_light` 同走黄）、`textPrimary = isLightExperiment ? neutralBlue950 : neutralWhite`、`surface = isLightExperiment ? neutralWhite : neutralCool900` … 语义名与调用点全部不变，仅底层改为引用 `AppPalette`。§A 分「中性外壳」（按 `isLightExperiment` 翻转，`pink_light`/`yellow_light` 共用）与「强调身份」（`accent`/`onAccent`/`accentText`/`accentSoft04/08`/`accentDisabledFill`/`lightCardBorder`，按 `themeId==pink_light` 选粉 vs 黄）。强调文字档 `AppBrandColors.accentText`：黄系主题取 `yellow700 #8F6400`（亮黄 `yellow500` 在白底不可读），`pink_light` 取 `pink600 #E03D74`。上层 `AppColors.accentText` = 当前主题面上可读的强调文字/图标：浅色态取 `AppBrandColors.accentText`，深色态保持亮强调色（`yellow500`）；覆盖分段选中字、年龄选项选中字、搜索动作字、链接（`linkSmallDark`）、点赞高亮图标、回复栏「发送」、验证码重发、签到今日奖励字等。福利页顶栏说明图标使用 `welfareHeaderInfoIcon`：浅色主题解析为 `blackAlpha100` 纯黑，深色主题解析为 `whiteAlpha60`。
>
> 语义别名（值不变，收敛登记补全）：`white00` / `white04` / `white05` / `white06` / `white08` / `white20` / `white24` / `white30` / `white50` / `white60` / `white85` / `white100` / `black00` / `black04` / `black08` / `black30` / `black40` / `black60` / `black80` / `black100` / `background` / `backgroundLight` / `surface` / `border` / `divider` / `textSecondary` / `textOnPrimary` / `primary` / `primarySoft` / `accentYellow` / `accentYellow04` / `navActiveText` / `sectionActionIcon` / `navActiveBackground` / `auroraGlow` / `auroraEdge` / `originalPriceMuted` / `planOriginalPrice` / `subtitleMuted` / `hotSaleBadge` / `hotSaleBadgeText` / `vipCtaGradientStart` / `vipCtaGradientEnd` / `vipCtaBorder` / `vipSelectedBorder` / `vipBannerText` / `brandBlueBackground` / `brandBlueAccent` 均为上述 `AppPalette` 原色的语义命名。
...
| Token | 不透明度 | ARGB | 用途 |
|-------|------|------|----|
| `blackAlpha100` | 100% | `0xFF000000` | 浅色主题需强对比的纯黑图标 |
...
| `borderSubtle` | `whiteAlpha04`（4% 白 `0x0AFFFFFF`） | `#F8E6ED` | `#F3F4F6` | 弱描边（深色态全局边框；黄色浅色态用更轻的中性浅灰） |
...
语义引用：`chromeBarScrim`（深 `bgTint80` / 浅 `white100`），`bottomNavScrim`（深 `backgroundDark` 不透明 / 浅 `white100` 纯白不透明），`bottomNavTextureScrim = bgTint60`（预留：纹理需透出时叠色，当前底栏默认用不透明 `bottomNavScrim` 盖住纹理），`topChromeBarScrolledScrim`（深 `bgTint80` / 浅 `white100` 纯白），`bottomActionBarScrim`（二级页底部固定操作栏：深 `bgTint80` 毛玻璃 / 浅 `white100` 纯白，书详情底栏、书评回复输入栏、装扮底栏、钱包底栏共用），用于压实全局毛玻璃、减少下方内容透出。两个浅色主题的顶部与底部 Chrome 统一为纯白 100% 实心底、**无 backdrop blur**；`AppPageChrome` 在浅色主题下顶栏**默认即启用**白底（不依赖滚动），深色主题仍仅在内容滚入顶栏后启用毛玻璃。`yellow_dark` 保留原毛玻璃效果。福利页未发生内容重叠时仍展示浅黄彩头，吸顶覆盖内容后切为纯白实底。`AppBottomNav` 铺 `AppThemeAssets.bottomNavTexture`（可选）+ 不透明 `bottomNavScrim`。
...
| `yellow_light` | `#F8F7FC`（`neutralCool50` 中性浅灰） | `#FFE847`（黄，复用深色主强调） | `#F8F7FC`（`neutralCool50Alpha*` 阶） | dialog/浮层 `#FFFFFF`；壳文字深墨 `#1A1A2E` | 黄色浅色系；壳背景中性浅灰（不偏黄），强调身份为黄；卡片细描边 `borderSubtle` 使用更轻的中性浅灰 `#F3F4F6` |
...
- **中性面/描边**：`surface`→`#FFFFFF`；`divider`→`#F3F4F6`；`borderSubtle`→ `lightCardBorder`（`pink_light` 浅粉 `#F8E6ED` / `yellow_light` 更轻的中性浅灰 `#F3F4F6`）；深色态 `surface` / `divider` 对应 `neutralCool900` / `neutralCool820` 实体中性色，`borderSubtle` 深色态取 `whiteAlpha04`（4% 白）。兼容别名 `surfaceCard` / `surfaceGlass` / `borderGlass` / `dividerOnDark` 指向这些核心语义。
- **双职责拆分（图上遮罩恒暗）**：`rankingHeroScrimTop`/`rankingHeroScrimMid`/`searchStatusBadgeBackground` 深色随主题壳基色渐隐；浅色态改用 `blackAlphaNN`，保证头图/封面上的白色文字仍可读（不随浅背景变浅）。背景渐隐类 `gradientFade*` 仍走 §A `bgTint`；Chrome 类在浅色主题收敛为 `white100` 实底且关闭模糊。
...
- **按钮禁用底**：`buttonDisabledFill` 深色态弱实体面（`_darkSurface`），浅色态 = `accentDisabledFill`（`pink_light` 玫粉 40% `0x66FF4D88` / `yellow_light` 黄 40% `0x66FFE847`）。禁用字色 `buttonDisabledText`：`pink_light` 玫粉底上 85% 白字（`white85`）；`yellow_light` 黄底上白字不可读，改用 30% 纯黑（`black30`）——保持「黑字降透明度」的禁用观感；`yellow_dark` 走三级字（`_darkTextTertiary`）。
- **选中胶囊**：~~`discussionFilterSelectedBackground` 用于讨论筛选~~ → 讨论筛选已改为分类同款纯文字（选中 `textPrimary` / 未选 `textTertiary`），不再使用面性胶囊；`discussionFilterSelectedBackground` 暂保留供兼容。
- **帖内标签**：`discussionTagBackground` = `surfaceSoft`（浅 `neutralCool50` / 深弱面），保证白页上可见实体底。
...
> 新增色值：`yellow500Alpha40 = 0x66FFE847`（`yellow_light` 按钮禁用底）；`yellow_light` 使用中性浅灰背景 `neutralCool50 #F8F7FC` 与白面板 `#FFFFFF`，卡片描边复用更轻的中性 `neutralCool100 #F3F4F6`。翻转/身份开关命名前缀：`_isLight` / `isLightExperiment` / `isYellowLight`（黄浅包单独判定，供按钮禁用字色等少量需区分 `pink_light` 的语义）/ `_ink*` / `_light*` / `_dark*` / `onAccent` / `accentSoft*` / `accentDisabledFill` / `lightCardBorder`。v1 已知未覆盖（后续视觉走查）：极光片元着色器如需浅色专属配色、少量 `whiteAlphaNN` 渐变遮罩/CTA 扫光（本为图/CTA 上光效，保留白色）。
...
| `checkInRewardTodayText` | `accentText`（深色亮黄 / 浅色深黄 `yellow700` 或深粉 `pink600`） | 今日奖励文字（8% 黄底上可读） |
| `checkInDayBg` | 深色 `whiteAlpha04` / 浅色 `surfaceSoft`（`_isLight` 翻转，浅底可见实体面） | 里程碑气泡底；普通签到日、阅读福利日与累计签到卡的内容区统一使用 `surfaceCard`，仅标题栏使用 `checkInDayHeader` 灰色填充 |
| `checkInCumulativeBorder` | `whiteAlpha00`（透明） | 时间轴可领取气泡描边（`checkInCumulativeBg` 已移除：福利页行内奖励角标统一 `taskRewardChipBg` 底 + `taskRewardChipText` 字，含 VIP 签到奖励 / 看视频等任务行） |
| `taskRewardChipBg` / `taskRewardChipText` | 随主题解析的弱面 / 主文字 | 福利页全局「图标 + 数值/时长」行内奖励标签；统一高度 `welfareTaskRewardChipHeight=22`，水平 padding `xs`、垂直 padding `xxs`，三个主题一致。时间轴已领取气泡的背景保持 100% 不透明，仅图标与文字降至 30% |
| `welfareReadingFreeCardWidth` / `Height` | `48 × 48` | 7 日阅读福利时间轴的限免卡透明画布展示框；允许溢出 36px 节点布局框以明显放大实际卡面，图片视觉层上移 `xs`，不改变轨道与文案位置 |
| `checkInProgressFill` / `checkInProgressDot*` | 进度橙 / 未达成灰色节点 | 累计签到进度按天数插值；达到 15 / 22 / 30 天时，对应节点切换为 `checkInProgressFill` 强调色，未达成节点保持灰色 |
| `checkInMilestoneAmount` | 深色 `whiteAlpha100` / 浅色 `textPrimary`（`_isLight` 翻转，浅底墨字可读） | 里程碑数值 |
| `planSelectedBg` | 深 `0x14FFE794`（8% 会员金）/ 浅 `0x4DFFE794`（描边金 30%） | 会员方案选中卡底；浅色态增强选中填充 |
...
| `planSelectedTextStart` / `planSelectedTextEnd` | 深 `#FFE794 → #FFCD5A` / 浅 `brown800 #5D3A12` 实色 | 选中卡标题与价格；浅色态提升对比度 |
...
| `planUnselectedBg` | 深 `whiteAlpha04` / 浅 `surfaceCard #FFFFFF` | 会员方案未选中卡底；浅色态纯白 |
| `searchHotAccent` | `#FF7E32`（accentOrange） | 搜索热词强调 |
| `bookDetailUpdateHighlight` | `#F0B16A`（`tan400`） | 账本正向数额等橙金强调（原「更新」时间线高亮色，时间线已改主文字） |
| `bookDetailUpdate*Highlighted` | `textPrimary`（浅黑 / 深白） | 书详情「更新」最新节点：日期 / 圆点 / 正文 |
| `bookDetailUpdateDate` / `bookDetailUpdateText` / `bookDetailUpdateDot*`（非高亮） | `textSecondary`/`textTertiary` 系（`textOnDarkMuted`） | 书详情「更新」历史节点：灰字 + 灰圆点 |
| `bookCoverBottomPopularityBackground/Icon` | `black60` / `orange500` | 封面右下热度标签的恒暗底与通用火花图标 |
| `bookCoverBottomPromotionBackground` / `bookCoverBottomBadgeText` | `orange500` / `white100` | 封面右下橙红运营标签与恒白文字；`bookCoverTagIconSize=12` |
| `rankingSegmentedSelectedText` | `#202020`（textOnLightPanel） | 榜单分段控件选中字 |
| `segmentedSelectedBorder` | `yellow_light`=`primary`；其余=`whiteAlpha00`（透明） | 年龄等强调选中描边（`ageRangeSelectedBorder` 别名）；与 `AppSegmentedSwitch` 解耦 |
| `segmentedSwitchSelectedFill` | 浅 `white100` / 深 `surface` | `AppSegmentedSwitch` 选中滑块白底 |
| `segmentedSwitchSelectedBorder` | `whiteAlpha00` | `AppSegmentedSwitch` 选中无描边 |
| `segmentedSwitchSelectedText` | `textPrimary` | `AppSegmentedSwitch` 选中字（浅黑 / 深白） |
...
| `discussionFilterUnselectedBackground` | `surfaceSoft`（深 `neutralCool920` / 浅 `neutralCool50`） | 历史筛选胶囊底（筛选已改纯文字；帖内标签仍用 `discussionTagBackground`） |
| `discussionFilterUnselectedText` | `textTertiary` | 书详情讨论筛选未选中文字（别名，字样式已对齐 `categoryFilterUnselected`） |
| `discussionTagBackground` | `surfaceSoft`（同未选胶囊） | 书详情讨论帖「公告」等弱面标签底 |
| `discussionAuthorBadgeBackground` / `discussionAuthorBadgeText` | `error` / `white100` | 讨论区昵称旁「作者」红标（区别于消息页黄底 `authorBadge*`） |
| `discussionPinnedTagGradientStart` | `orange550 #FF6F4B` | 讨论「置顶」渐变起色（左上偏黄红） |
| `discussionPinnedTagGradientEnd` | `rose500 #FF4E6C` | 讨论「置顶」渐变止色 |
| `discussionFeaturedTagGradientStart` | `pink200 #FF9CC7` | 讨论「精选」渐变起色（左上淡粉紫） |
| `discussionFeaturedTagGradientEnd` | `purple400 #9C87FF` | 讨论「精选」渐变止色 |
| `discussionContentTagText` | `white100` | 置顶/精选渐变标签字 |
| `discussionItemReplyBackground` | `surfaceSoft`（深 `neutralCool920` / 浅 `neutralCool50`） | 书详情讨论回复摘要底 |
| `discussionNewCommentHighlight` | `pink500Alpha08`（品牌粉 8%，全主题恒定） | 讨论新发评论短暂高亮底（通栏、无圆角） |
...
| `tabTopHeaderGradientStart` / `tabTopHeaderGradientEnd` | `yellow_light`：`primary #FFE847` → `white00`；其余主题：`white00` → `white00`（不可见） | 福利页头部垂直渐隐装饰（`AppTabTopTexture` 无贴图时铺设；用户指定仅浅黄主题的福利页显示，书城首页/书架不调用） |
| `dialogTopHeaderGradientStart` / `dialogTopHeaderGradientEnd` | `yellow_light`：`yellow500Alpha40` → `white00`；其余主题：`white00` → `white00`（不可见） | 弹窗顶部彩头（`AppDialogTopTexture`）：较页头更淡的 40% 黄 → 透明垂直渐隐 |
| `dialogTopTextureHeight` | `120` | 弹窗顶部彩头高度（`AppDialogTopTexture`） |
| `bookshelfPageBackground` | = `backgroundDark`（浅 `neutralCool50`/`pink50` / 深壳底） | 书架整页底色（非纯白，供白渐变下透出） |
| `bookshelfPageGradientStart` / `bookshelfPageGradientEnd` | `white100` → `white00`（三主题） | 书架顶部随滚垂直白渐变起止 |
| `bookshelfReadingMinutes` | `hotSaleBadge`（`red400`） | 书架「今日已阅读」分钟数标红 |
...
| 次按钮 `AppButton(secondary)` | 浅 `surfaceSoft` / 深 `surface` · `textPrimary` |
...
| 顶栏 / 底栏 Chrome（深色毛玻璃、浅色实底） | `chromeBarScrim` / `topChromeBarScrolledScrim` / `bottomNavScrim` / `bottomActionBarScrim` |
| 底部导航 | 底 `navBarBackground`（浅色 `white100` 实底 / 深色 `surface`）· 选中/未选主题色稿图标（`AppThemeAssets`）+ 选中标签 `textPrimary`（浅色深墨 / 深色纯白）· 未选标签 `iconMutedSecondary` |
| 输入框 / 搜索 | 光标 `searchCursor`(=primary) · 「搜索」动作字 `searchActionText`(=`accentText`，深色亮黄 / 浅色深黄或深粉) · 文字 `textPrimary` · 占位 `textTertiary` |
| 弹窗 | 底 `surfaceElevated` · 遮罩 `overlayScrim80` |
| 分段控件 | 轨道底 `surfaceSoft` · 选中滑块 `segmentedSwitchSelectedFill`（白）· 选中字 `segmentedSwitchSelectedText`（黑/主文字）· 无描边 `segmentedSwitchSelectedBorder`；年龄选项仍用 `segmentedSelectedFill` + `segmentedSelectedBorder` |
...
| 分组 | 覆盖范围 | Token 前缀 / 示例 |
|------|----------|------------------|
| 通用基础 | 描边 / 通用图标 / 启动图（底部品牌 Logo 占屏宽 68.5%、最大 315px，安全区上方留白 `lg`）/ 标题说明图标（20px）/ 区块尾随箭头 12 | `neutralCool200` · `borderWidthEmphasis` · `iconSm` · `titleInfoIconSize` · `sectionTrailingIconSize` · `splashLogoSize` · `splashLogoWidthRatio` |
...
| 讨论新评论高亮 | 淡粉底停留 | `AppDurations.discussionNewCommentHighlight`（5s） |
...
| 福利页 | 头图 / 签到里程碑 / 任务时间线 / 充值弹窗；签到成功弹窗顶边条高 10、侧纹 `checkInSuccessSideStripe` 透明度 0.45、外飘星 `checkInSuccessSparkle` | `welfare*` · `rechargePurchaseDialog*` · `checkInSuccessSideStripe` · `checkInSuccessSparkle` · `welfareCheckInSuccessTopAccentHeight` · `welfareCheckInSuccessSideStripeOpacity` · `welfareCheckInSuccessSparkle*` |
| 书架页 | 顶栏 / 阅读横幅吸顶固定（无插画、无底填充、高 `bookshelfClaimWelfareCtaHeight` 32；分钟数 `bookshelfReadingMinutes` 标红）/ 空状态 / 封面角标；整页底 `bookshelfPageBackground`（壳基色）+ 随滚白渐变 `bookshelfPageGradient*`（高 500）；书卡无卡面底填充；顶栏→横幅 `bookshelfHeaderToBannerGap` 4、横幅→书格 `bookshelfBannerToGridGap` 12 | `bookshelf*` · `bookCoverTag*`（含 `bookCoverTagBlurSigma`） |
| 我的页 | Hero / 头像 / 快捷入口 / 成就勋章 | `profile*` · `homeIndicator*` · `listRowMinHeight` |
| 我的-子页 | 账号设置 / 消息 / 卡包 | `accountSettings*` · `myMessages*` · `cardPack*` · 互动消息标识 `authorBadge*`（复用品牌黄）· 讨论区作者标 `discussionAuthorBadge*`（系统红）· 未读红点 `badgeCount`（复用 rose400 红）· 通知标识 `myMessagesNoticeBadge`（复用品牌橙）|
| 设置页 | Logo / 开关 `AppSwitch` | `settings*` · `appSwitch*` |
| 书籍详情页 | 头图 / 目录抽屉 / 角色卡 / 讨论区 / 更新时间线；整页底 `bookDetailPageBackground`（浅色纯白 / 深色深底）；讨论「写评论」FAB `bookDetailWriteCommentFabSize` 80（240px @3x）、无促销条时底距 `bookDetailWriteCommentFabBottomGap` 12；新评论高亮 `discussionNewCommentHighlight`（粉 8% `pink500Alpha08`、通栏无圆角）、滚入对齐 `bookDetailNewCommentScrollAlignment` 0.2 | `bookDetail*` · `bookDiscussionDetail*` · `bookDetailPageBackground` · `bookDetailWriteCommentFab*` · `bookDetailNewCommentScrollAlignment` · `discussionNewCommentHighlight` |
...
| 分类页 | 筛选组 / chip / 下划线；筛选区底部与书单间距 `categoryFilterSectionVerticalPadding` 24；完整筛选区滑出后在顶部显示当前条件摘要，点击平滑回到顶部并重新展示完整筛选；分类/排行整页底色统一 `categoryRankingPageBackground`（浅色主题纯白，黄黑主题保持深底可读） | `categoryFilter*` |
...
| `secondary` | 浅色 `surfaceSoft` 极轻灰底 / 深色 `surface` · 无描边 · 胶囊 `full` | 次操作 / 弱化 / 未激活态（重试默认、退出登录、验证码倒计时、签到成功「看视频再领」）·**默认变体** |
...
**状态：** default · `isLoading`（转圈，保留变体外观）· **disabled（不可点击：`onPressed==null` 且非 loading）→ 全局统一 `buttonDisabledFill` + `buttonDisabledText`，无渐变、无描边，覆盖所有变体；主题感知：深色态弱实体面（`_darkSurface`）+ 三级字，浅色态半透明强调色底（`pink_light` `pink500Alpha40` + 85% 白字 `white85` / `yellow_light` `yellow500Alpha40` + 30% 纯黑字 `black30`）** · `isExpanded`（撑满宽度）。**可选：** `leadingIcon` 前置图标；`iconLabelGap` 可覆写图标与文字间距（默认 `buttonIconLabelGap`，紧凑场景用 `buttonIconLabelGapTight`）。描边一律 0.5px neutralCool200。
...
| `BookCoverBottomBadgeView` | `shared/components/book_cover_bottom_badge.dart` | 标准书籍封面右下角运营标签，由后端 `coverBottomBadge` 驱动；`popularity`=黑 60% 底 + 通用火花图标 + 白色数值，`promotion`=橙红底 + 白色文案；统一通过 `BookCover.bottomEndBadge` 叠加，右/下 inset=`xxs`。例外：推荐页顶部 `RankingBookGrid` 的紧凑小封面不显示，避免遮挡封面 |
...
| `AgeRangeOption` | `shared/components/` | 年龄段单选胶囊（整行，`horizontal md` / `vertical xs`）：选中 `segmentedSelectedFill`（8% 强调底）+ 强调文字档 `accentText` **semibold** + `segmentedSelectedBorder`（`ageRangeSelectedBorder` 别名；仅 `yellow_light`=`primary` + `borderWidthEmphasis`，`yellow_dark` / `pink_light` 透明描边）。未选 `surfaceSoft` + `borderSubtle` 细描边 + `textPrimary` 字，圆角 `full`；新手弹窗与偏好设置页共用（高度/选中样式统一）|
...
| `OnboardingProfileDialog` | `features/onboarding/presentation/pages/` | 新用户首页首启弹窗：性别 → 年龄两步**横向切换**（高度固定、内容区无上下滚动、底部分页器可回退，右上角统一 `DialogCloseButton` X，标题统一 `titleMedium`，底部固定「完成」）；性别使用共用 `GenderAvatarOption`（圆形头像 80px，**左右并排**，选中彩色 + 黄描边环、未选灰 + 60% 白字，与偏好设置页一致），选中性别后短暂停留展示反馈再切到年龄；年龄使用共用 `AgeRangeOption`（8% 强调底 + `accentText` 加粗字，`yellow_light` 额外黄色强调描边，与偏好设置页统一）|
| `ReadingPreferencesPage` | `features/settings/presentation/pages/` | 偏好设置页（二级页）：性别（共用 `GenderAvatarOption` 头像，左右并排）+ 年龄（共用 `AgeRangeOption` 胶囊单选，8% 强调底 + `accentText` 加粗字，`yellow_light` 额外黄色强调描边，与新手弹窗统一）；底部「保存」|
...
| 确认壳 | `AppConfirmDialog`（`shared/components/app_confirm_dialog.dart`，L2）— 面板 chrome + 标题/正文 + 双按钮（默认左次按钮 `secondary` / 右主按钮 `accent`；可覆写变体、单主按钮、关闭钮）；`yellow_light` 顶部叠 `AppDialogTopTexture`；业务 L3 只填文案 |
| 业务示例 | `EnergyRechargePurchaseDialog` / `WelfareRulesDialog` / `DailyCheckInDialog` → `CheckInSuccessDialog`（签到类弹窗统一装饰：`check_in_dialog_decor` 顶边条 10 + 侧纹 0.45 + 外飘/标题星）（L3） |
...
选项：`showScrim`（hero 沉浸渐变蒙版）/ `chromeBlurEnabled`（毛玻璃）/ `height` / `horizontalPadding`。配套：`AppTopBarIconButton`、`AppTopBarTextButton`、`AppBlurredChromeBar`。三个主题统一：右侧为图标动作时使用 `AppTopBarIconButton` 默认裸图标样式，不绘制背景填充或圆框，但保留统一点击热区；仅左侧返回按钮传 `showFrame: true` 保留圆框（浅色 `white100` 实底无模糊，深色保持磨砂）。
...
| `AppAnimatedTabLabel` | `shared/components/app_animated_tab_label.dart` | Tab 文字标签：未选中↔选中样式（字号/字重/颜色）用 `TextStyle.lerp` 平滑过渡而非硬切；与 `ElasticTabIndicator` 同一进度模型——传 `swipeProgress` 随手指连续插值，未传则选中项变化时按 `AppDurations.normal` 平滑过渡（仅 from/to 两端交叉过渡，不扫过中间索引，跨档点击中间 Tab 不闪选中态）。`AppTopTabBar` 及变宽 `ElasticTabRow`（书架 / 装扮）Tab 共用 |
...
| 组件 | 位置 | 变体 / 说明 |
|------|------|------|
| `SectionHeader` | `shared/components/section_header.dart` | 区块标题 + 可选 `titleSuffix` + 右侧尾随操作（内部复用 `SectionTrailingAction`） |
| `SectionTrailingAction` | `shared/components/section_trailing_action.dart` | 区块右侧文案 + 右箭头（目录状态 /「滑动查看更多」/「更多」等同款） |
...
| `AppConfirmDialog` | `shared/components/app_confirm_dialog.dart` | 居中确认弹窗壳（见 §7.3）；`yellow_light` 顶部叠 `AppDialogTopTexture` |
...
| `AppSelectionMark` | `shared/widgets/app_selection_mark.dart` | 圆形多选勾选标记：选中黄底 + 深色勾（`selectionMarkSize` 圆 + `bookshelfSelectionCheckIconSize` 勾）、未选透明底 + 描边（`bookshelfSelectionMarkBorderUnselected`）；**封面上**（书架管理态）未选中态泛白覆写（用户指定，keep-dark 恒白系）：`selectionMarkOnCoverUnselectedFill`（30% 白底）+ `selectionMarkOnCoverUnselectedBorder`（85% 白描边），三主题一致；页面面上（登录协议 / 支付方式 / 全选）保持默认样式 |
...
| `CurrencyBalanceBar` / `RechargePackagesSection` / `VipPromoBanner` | `shared/components/` | 业务复用组合组件；余额统一顺序为「星尘 → 能量 → 祈愿星 → 爱心」。`CurrencyBalanceBar` 仅在福利页的 `yellow_light` 场景开启 `useTransparentYellowLightStyle`，移除白色填充、描边与模糊，首末项分别贴齐内容区两端，中间项均匀分布；“我的”页始终保留白色卡片填充、内边距、描边与模糊。`yellow_light` 下星尘数字上方显示紧凑红色「可兑换能量」标签（水平/垂直 padding `xxs`），底部小三角（`sm × xxs`）向上回缩 `xxsHalf` 并居中贴近数字；白卡仅裁剪背景模糊层，标签内容不裁剪，复用 `hotSaleBadge` + `welfareHotSaleBadge`。`yellow_dark` / `pink_light` 保留白卡结构且不显示该标签。其余福利外层白卡统一使用 `borderGlass` + `hairline` 弱描边；充值区价格按钮与「免费领 / VIP领取」CTA 共用同款暗色胶囊（`surface` 底、`welfareRechargePrice` 圆角、`welfareRechargePriceButtonHeight` 高），价格按钮用单段落 rich text 使 `¥` 与数字底对齐 |
...
### 9.3 呼吸 / 循环 / 引导 / 加载
- 书城下拉刷新奔跑小熊（`features/bookstore/presentation/components/bookstore_refresh_visual.dart`）：20 帧透明 PNG 在推荐页进入时预缓存，避免首次拖动解码停滞；`drag / armed / refresh / done` 全部可见阶段均以 0.8 秒周期持续循环，仅在 `inactive` 完全收起后停止。上拉阶段按进度从 36px 基础视觉尺寸平滑放大至 50px，形成向用户跑近的纵深感，最小尺寸保持清晰；松手后弹簧先收敛到固定停留态，最短展示 1.6 秒，保证动画完整循环两次，刷新完成后统一回弹消失；视觉下移 `AppSpacing.xxl + AppSpacing.xl + AppSpacing.lg`，刷新占位高度为 `chromeTopHeight - AppSpacing.md`，触发距离保持不变；三主题共用。
...
- 玻璃组件：`GlassChipButton`、`AppSegmentedSwitch`、`AppBottomNav`（glassCapsule）、`AppTopBarIconButton`、`CurrencyBalanceBar`、会员用户卡、伙伴筛选弹层。
- 强模糊：福利任务叠层（`strongBlurSigma`）。
- 继续阅读浮条：全主题保持深色壳样式（`continueReadingCard*` keep-dark，不随浅色包翻转）；页面背景色之上叠「放大封面」背景层（宽度撑满卡片、上下左右居中、半透明 `continueReadingBgImageOpacity` + 模糊 `continueReadingBgBlurSigma`），不同书籍呈现不同底纹；左侧封面缩略图放大并向上溢出卡片顶部（外层 `Stack` 不裁剪 + 投影 `continueReadingCoverShadowBlur`），呈悬浮抬起效果。福利待领取气泡可见时经 `MainTabPendingClaimScope` 额外上移 `AppSpacing.xs`（8px，与气泡顶相距），气泡消失后按 `AppDurations.normal` 归位。
...
- 倒计时：短信 / 福利任务 `HH:MM:SS`（未归零时动作按钮禁用文案「暂未开启」，归零后恢复）；续费提示 slot 交叉淡入 + 高度动画。
...
### 尚未实现（后续增强候选）
- **Lottie**：底栏 `yellow_light` 已接入（`assets/lottie/bottom_nav/yellow_light/`）；其它场景继续经 `AppLottie` 扩展。
