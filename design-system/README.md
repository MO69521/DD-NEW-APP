...
| `tabTopHeaderGradientStart` / `tabTopHeaderGradientEnd` | `yellow_light`：`primary #FFE847` → `white00`；其余主题：`white00` → `white00`（不可见） | 一级 Tab 头部垂直渐隐装饰（`AppTabTopTexture` 无贴图时铺设，用户指定仅浅黄主题显示） |
...
| 顶栏 / 底栏毛玻璃 | `chromeBarScrim` / `topChromeBarScrolledScrim` / `bottomNavScrim` / `bottomActionBarScrim` |
...
| `AppTabTopTexture` | `shared/components/app_tab_top_texture.dart` | 一级 Tab（书城/福利/书架）顶部装饰纹理；全宽 × `height`（默认 `tabTopTextureHeight` 120，页面可传语义 token 覆写——福利页 `welfareTabTopTextureHeight` 300，用户指定）；`AppThemeAssets.tabTopTexture` 为 null 时铺 `tabTopHeaderGradient*` 头部渐变——仅 `yellow_light` 可见（顶部主黄 `primary` → 底部白 0% 垂直渐隐，用户指定），`yellow_dark` / `pink_light` 起止均透明等同原透明槽位 |
