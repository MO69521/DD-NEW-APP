...
| 透明效果原语 | `whiteAlpha100…whiteAlpha00` / `blackAlpha80…blackAlpha00`（仅用于遮罩、蒙版、扫光、渐隐等 effect；不作为组件底色 / 文本 / 描边色库） |
...
| 壳基色 · 粉浅 | `pink50 #FFF5F9` + `pink50Alpha00` / `pink50Alpha35` / `pink50Alpha45` / `pink50Alpha55` / `pink50Alpha60` / `pink50Alpha80` / `pink50Alpha90` |
| 深壳装饰 | `cream100 #FFF2C6` · `wine950 #1D0B10` · `neutralCool880 #131820` · `neutralCool800 #262B33` |
| 主强调 | `yellow500 #FFE847`（+ `yellow500Alpha04`/`yellow500Alpha08`）· `pink500 #FF4D88`（+ `pink500Alpha04`/`pink500Alpha08`） |
...
> 仅用于扫光、高光、图片蒙版、渐隐等 effect；组件底色 / 文本 / 描边不得直接以 `whiteAlphaNN` 作为设计依据，应使用 §4.2 语义 token。
...
| `whiteAlpha04` | 4% | `0x0AFFFFFF` | 低透明叠加（effect） |
...
| `borderSubtle` | `#252B34` | `#F4D9E4` | 弱描边 |
...
深色态已改为实体中性色（`neutralCool900` / `neutralCool800` / `neutralCool820` / `neutralWhite` / `neutralCool400/500`）承载组件底色、描边与文本；白/黑透明阶仅用于 effect / overlay / mask。浅色实验包（`pink_light`）在 `AppColors` 用 `_isLight`（源 `AppBrandColors.isLightExperiment`）把以下语义 token 翻到浅底取值。

- **文字**：`textPrimary`→`#1A1A2E` / `textSecondary`→`#6B7280` / `textTertiary`→`#9B9B9B`；兼容别名 `textOnDark*` 随这些语义一起翻转。
- **中性面/描边**：`surface`→`#FFFFFF`；`borderSubtle`→`#F4D9E4`；`divider`→`#F3F4F6`；深色态对应 `neutralCool900` / `neutralCool800` / `neutralCool820` 实体中性色。兼容别名 `surfaceCard` / `surfaceGlass` / `borderGlass` / `dividerOnDark` 指向这些核心语义。
...
- **底部导航**：选中态靠图标满色 + 标签 `primary`（自动随主强调翻粉）+ 未选中标签中性灰；`aurora` 背景默认走 `primary`。
...
> 新增色值：`#FFF5F9`（背景）、`#F4D9E4`（浅粉描边）、`#FF4D88` 低透明派生（`0x0AFF4D88` / `0x14FF4D88`）；其余复用既有源色。翻转开关/源值命名前缀：`_isLight` / `_ink*` / `_light*` / `_dark*` / `onPrimary`。v1 已知未覆盖（后续视觉走查）：极光片元着色器如需浅色专属配色、少量 `whiteAlphaNN` 渐变遮罩/CTA 扫光（本为图/CTA 上光效，保留白色）。
...
| `checkInDayBg` | `whiteAlpha04`（effect 低透明，待后续 feature 二期收敛到实体面） | 普通签到日底 |
| `checkInCumulativeBg` | `yellow500Alpha08`（8% primary） | 累计签到徽章底 |
| `checkInCumulativeBorder` | `whiteAlpha00`（透明） | 累计徽章描边 |
| `checkInMilestoneAmount` | `whiteAlpha100` | 里程碑数值 |
...
| `planSelectedSecondary` | `whiteAlpha60` | 选中卡次要文字 |
...
| `discussionItemReplyBackground` | `neutralCool920` | 书详情讨论回复摘要底 |
| `discussionLikeIcon` | `textTertiary` | 书详情讨论点赞图标默认色 |
| `guessLikeCardBackground` | `neutralCool920` | 猜你喜欢卡片底 |
| `guessLikeTagBackground` | `neutralCool900` | 猜你喜欢标签底 |
| `guessLikeTagBorder` | `neutralCool800` | 猜你喜欢标签描边 |
...
**状态：** default · `isLoading`（转圈，保留变体外观）· **disabled（不可点击：`onPressed==null` 且非 loading）→ 全局统一 4% 纯白填充（`buttonDisabledFill`）+ 30% 白字（`buttonDisabledText`），无渐变、无描边，覆盖所有变体** · `isExpanded`（撑满宽度）。**可选：** `leadingIcon` 前置图标；`iconLabelGap` 可覆写图标与文字间距（默认 `buttonIconLabelGap`，紧凑场景用 `buttonIconLabelGapTight`）。描边一律 0.5px neutralCool200。
