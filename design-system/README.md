...
全项目唯一允许 `Color(0x…)` 字面量的地方；只放原色、无语义、不分主题。新增原色必须登记此处。一览（§4.1.1）为实体/品牌/状态原色；中性透明阶明细见 §4.1.2（白）/ §4.1.3（黑）。

#### 4.1.1 实体 / 品牌 / 状态原色一览

| 分组 | 原色（AppPalette 名 → hex） |
|------|------|
| 中性白阶 | `white100…white00`（见 §4.1，值不变） |
| 中性黑阶 | `black80…black00`（见 §4.2） |
| 浅色实体 | `ink #1A1A2E` · `inkMuted #6B7280` · `inkFaint #9B9B9B` · `line #F3F4F6` · `hairline #E5E7EB` · `pinkBorder #F4D9E4` · `paperCool #F8F7FC` |
| 壳基色 · 深 | `shellDark #090E17` + `shellDarkT00/T35/T45/T55/T60/T80/T90`（透明度阶） |
| 壳基色 · 粉浅 | `shellPink #FFF5F9` + `shellPinkT00/T35/T45/T55/T60/T80/T90` |
| 深壳装饰 | `auroraGlow #FFF2C6` · `auroraEdge #1D0B10` · `dialogDark #131820` · `surfaceMutedDark #262B33` |
| 主强调 | `brandYellow #FFE847`（+ `brandYellow04`/`brandYellow14`）· `brandPink #FF4D88`（+ `brandPink04`/`brandPink14`） |
| 面板深字/灰 | `onLightPanel #202020` · `grayStrike #919191` · `graySubtitle #8C8C8C` · `iconGray #B2B3BA` · `iconGray2 #ABACB3` · `emptyGray #757575` |
| VIP 粉紫 | `vipGradStart/vipGradEnd/vipOnGradText/vipCtaStart/vipCtaEnd/vipCtaBorder/vipSelectedBorder/vipBannerText`（值见 §4.2.2） |
| 伙伴粉 | 主色 = `brandPink` · `partnerLight #FF7AA8` · `partnerDark #E03D74` |
| 福利金/橙 | `goldMedium/goldDark/goldText` · `orange #FF7E32` · `checkInYellow #FCE64D` · `checkInHeader #FFDD47` |
| 促销条 | `promoStart/promoMid/promoEnd/promoSubtitle/promoReward`（值见 §4.2.2） |
| 榜单头图 | `rankingTitle #FFFAD7` · `rankingSubtitle #E6FFFAD7` |
| 书详情更新 | `bookUpdateHighlight #F0B16A` |
| 语义状态 | `success #39D98A` · `warning #FFA940` · `error #FF667F` · `hotSale #FF6B6B` · `hotSaleText #FFFEF4` |
| 蓝色预设 | `blueBg #0A1628` · `blueAccent #4DA6FF`（礼花粒子 / 休眠 `AppColorScheme.brandBlue`） |
| v1.0 扩展 | `premiumGold #F9C74F` · `info #59AEFF` · `fantasyPurple #9C87FF` · `energyCyan #42DDFF` · `growthBlue #7E95FF` |

> 上层映射：`AppBrandColors.partnerPrimary = brandPink`、`accent = pink_light ? brandPink : brandYellow`、`AppColors.textOnDark = pink_light ? ink : white100` … 语义名与调用点全部不变，仅底层改为引用 `AppPalette`。
>
> 语义别名（值不变，收敛登记补全）：`background` / `backgroundLight` / `surface` / `border` / `textSecondary` / `textOnPrimary` / `navActiveBackground` / `originalPriceMuted` / `planOriginalPrice` / `subtitleMuted` / `hotSaleBadge` / `hotSaleBadgeText` / `vipCtaGradientStart` / `vipCtaGradientEnd` / `brandBlueBackground` / `brandBlueAccent` 均为上述 `AppPalette` 原色的语义命名。

#### 4.1.2 中性 · 白透明阶（`whiteNN`，ARGB 原值）
...
多风格小流量实验用**编译期整包换皮**，只覆盖「§A 主题壳源色」，其余 §B 品牌色恒定；**不提供用户运行时切换**。默认（不带参）永远是 `dark`，其源值不得改动。机制与加包步骤见 §8.3 与 `app_brand_colors.dart` 文件头配方。各 token 在 dark / pink_light 下的解析对照，可视化见 `design-system-spec.canvas.tsx` 的「多风格」tab（04）。

| THEME | backgroundDark | accent | bgTint 基色 | aurora/dialog/surfaceMuted | 备注 |
|-------|----------------|--------|------------|----------------------------|------|
| `dark`（默认） | `#090E17` | `#FFE847`（黄） | `#090E17` | 见 §4.2.2 | 默认深色壳，源值锁定 |
| `pink_light` | `#FFF5F9`（暖粉白，新增） | `#FF4D88`（玫粉，复用 `partnerPrimary`） | `#FFF5F9`（同基色标准透明度阶） | dialog/浮层 `#FFFFFF`；壳文字深墨 `#1A1A2E` | 粉色浅色系；需 §4.2.4 中性翻转层 |

> 实现命名：各实验包的 §A 源值以包 id 为前缀命名（如 `_dark*` / `_pinkLight*`），集中在 `app_brand_colors.dart` §A；公开字段（`backgroundDark` / `accent` / `bgTint*`）经 `themeId` 三元在这些前缀源值间选择。

#### 4.2.4 浅色实验包 · 中性翻转层（`_isLight` / `isLightExperiment`）

深色态用「白色半透明叠加」（`whiteNN`）做卡片/玻璃/分割，用白字做正文——都假设深底，浅底会失效。浅色实验包（`pink_light`）在 `AppColors` 用 `_isLight`（源 `AppBrandColors.isLightExperiment`）把以下语义 token 翻到浅底取值；默认深色 `_isLight=false`，所有翻转项走 else 分支，取值与深色态**逐字节一致**。

- **ink 文字**：`textOnDark`→`#1A1A2E`（`_inkPrimary`，复用 `textPrimary`）/ `textOnDarkMuted`→`#6B7280`（`_inkMuted`）/ `textOnDarkPlaceholder`→`#9B9B9B`（`_inkPlaceholder`）；`sectionActionIcon`（section 操作图标，深色 `white60`）随 placeholder 一同翻转。
- **中性面/描边**：`surfaceCard`/`surfaceGlass`→`#FFFFFF`（`_lightCard`）；`borderGlass`→`#F4D9E4`（`_lightBorder`，新增浅粉描边）；`dividerOnDark`→`#F3F4F6`（`_lightDivider`）；`navBarBackground`/`shimmer*`→黑色低透明（`blackNN`）。
- **双职责拆分（图上遮罩恒暗）**：`rankingHeroScrimTop`/`rankingHeroScrimMid`/`searchStatusBadgeBackground` 深色随主题壳基色渐隐；浅色态改用 `blackNN`，保证头图/封面上的白色文字仍可读（不随浅背景变浅）。「背景渐隐 / 毛玻璃底」类（`chromeBarScrim`/`gradientFade*` 等）仍走 §A `bgTint`，跟随浅色。

- **accent 上的文字/图标**：新增语义 token `onAccent`（深色态深墨 `#090E17`，即原 `navActiveText`；浅色态白 `#FFFFFF`）。原散用 `navActiveText` 的 accent 面场景统一改指 `onAccent`：主按钮 `onPrimary`、`authorBadgeText`、选中勾 `AppSelectionMark`、封面「更新」角标、`toastMessage`、`bookshelfClaimWelfareCta`、`bookDetailDiscussionFilterSelected`。
- **accent 派生填充**：`accentYellow04`（4%）、`segmentedSelectedFill`（8%）深色态为品牌黄叠加，浅色态随主强调翻为玫粉叠加（`0x0AFF4D88` / `0x14FF4D88`，即 `#FF4D88` 同色低透明）。
- **选中白胶囊**：`discussionFilterSelectedBackground` 深色态白 `#FFFFFF`，浅色态翻为 accent（玫粉）胶囊 + `onAccent` 白字。
- **底部导航**：选中态靠图标满色 + 标签 `accentYellow`（自动随主强调翻粉）+ 未选中标签中性灰，无需额外处理；`aurora` 背景默认走 `accentYellow`（自动翻粉），亦无需改。
- **主 CTA 文字**：`AppButton` 的 `accent` 变体前景深色态深字 `#202020`（黄底，不变），浅色态白字（粉底可读）。
...
> 新增色值：`#FFF5F9`（背景）、`#F4D9E4`（浅粉描边）、`#FF4D88` 低透明派生（`0x0AFF4D88` / `0x14FF4D88`）；其余复用既有源色。翻转开关/源值命名前缀：`_isLight` / `_ink*` / `_light*` / `onAccent`。v1 已知未覆盖（后续视觉走查）：极光片元着色器如需浅色专属配色、少量 `whiteNN` 渐变遮罩/CTA 扫光（本为图/CTA 上光效，保留白色）。
...
| `bookDetailPromoRewardText` | `#E64D00`（promoRewardText） | `+N` 能量角标文字（「立即领取」按钮为切图，见 §4.2.2 说明） |
...
| 组件 / 场景 | 引用的语义 token |
|------|------|
| 主按钮 `AppButton(accent)` | 底 `accent` · 字 `onAccent` |
| 卡片 / 玻璃面 | 面 `surfaceCard` / `surfaceGlass` · 描边 `borderGlass` · 标题 `textOnDark` |
| 分组列表 / 分割 | `dividerOnDark` |
| 顶栏 / 底栏毛玻璃 | `chromeBarScrim` / `topChromeBarScrolledScrim` / `bottomNavScrim` |
| 底部导航 | 底 `navBarBackground` · 选中 `accent` + 满色图标 · 未选 `iconMutedSecondary` |
| 输入框 / 搜索 | 光标 `searchCursor`(=accent) · 文字 `textOnDark` · 占位 `textOnDarkPlaceholder` |
| 弹窗 | 底 `dialogBackground` · 遮罩 `overlayScrim80` |
...
3. **换色系（编译期整包，小流量实验用，不提供运行时切换）**：`AppBrandColors` 分两区——**§A 主题壳源色**（`backgroundDark` / `accent` / `auroraGlow` / `auroraEdge` / `dialogBackground` / `textOnDark` / `surfaceMuted` / `bgTint00…90`，随 `THEME` 切换的唯一入口）与 **§B feature 品牌色**（VIP / 伙伴 / 福利金 / 语义状态色等，跨深壳实验恒定）。新增实验包：给 §A 每个字段的三元**前插**一条 `themeId == '<id>' ? _<id>源值 : _dark源值` 分支（新增 `_<id>*` 源值常量），构建时 `--dart-define=THEME=<id>`（Android flavor / iOS scheme / 渠道分包各固定一个 THEME）。`AppColors` / `AppColorScheme` / 页面均经 §A 取色，换包不改任何调用点。**默认（不带参）永远解析为 `dark`，§A 的 `_dark*` 源值与其默认分支不得改动。**
