...
| `dialogTopTextureHeight` | `120` | 浅黄主题福利规则弹窗顶部黄色彩头高度；复用 `tabTopHeaderGradientStart/End` 向下渐隐，其余主题透明 |
...
| 分段控件 | 选中底 `segmentedSelectedFill` · 选中字 `segmentedSelectedText`（深色态=accent 亮黄；浅色态=`accentText`：`yellow_light` 深黄 `yellow700`、`pink_light` 深粉 `pink600`，白面上可读） |
...
| 分组 | 覆盖范围 | Token 前缀 / 示例 |
|------|----------|------------------|
| 通用基础 | 描边 / 通用图标 / 启动图（底部品牌 Logo 占屏宽 68.5%、最大 315px，安全区上方留白 `lg`）/ 标题说明图标（20px） | `neutralCool200` · `borderWidthEmphasis` · `iconSm` · `titleInfoIconSize` · `splashLogoSize` · `splashLogoWidthRatio` |
...
| 分类页 | 筛选组 / chip / 下划线；完整筛选区滑出后在顶部显示当前条件摘要，点击平滑回到顶部并重新展示完整筛选；分类/排行整页底色统一 `categoryRankingPageBackground`（浅色主题纯白，黄黑主题保持深底可读） | `categoryFilter*` |
...
| `AgeRangeOption` | `shared/components/` | 年龄段单选胶囊（整行，`horizontal md` / `vertical xs`）：选中 `segmentedSelectedFill`（8% 强调底）+ 强调文字档 `accentText` **semibold**；仅 `yellow_light` 额外使用 `ageRangeSelectedBorder=primary` + `borderWidthEmphasis` 黄色描边增强反馈，`yellow_dark` / `pink_light` 保持透明描边。未选 `surfaceSoft` + `borderSubtle` 细描边 + `textPrimary` 字，圆角 `full`；新手弹窗与偏好设置页共用（高度/选中样式统一）|
...
| `AppSegmentedSwitch` | `shared/components/app_segmented_switch.dart` | 玻璃分段 Tab，选中滑块动画（书架 / 详情内 Tab）；选中态**无描边**，仅 `segmentedSelectedFill`（8% 强调底）+ 强调字区分（深色亮黄 / 浅色深黄 `yellow700` 或深粉 `pink600`） |
