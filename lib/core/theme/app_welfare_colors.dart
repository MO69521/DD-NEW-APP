...
abstract final class AppWelfareColors {
  ...
  static const Color checkInTodayHeaderText =
      _isLight ? AppColors.onPrimary : AppBrandColors.textOnLightPanel;

  /// 今日可领奖励文字（主黄）。
  static const Color checkInRewardTodayText = AppBrandColors.checkInYellow;
  // 普通签到日 / 里程碑气泡 / 累计卡底：深色 4% 白叠底，浅色翻为弱实体面。
  static const Color checkInDayBg =
      _isLight ? AppColors.surfaceSoft : AppColors.white04;
  static const Color checkInDayHeader =
      _isLight ? AppColors.surfaceSoft : AppColors.white04;
  ...
  static const Color checkInDayLabelMuted =
      _isLight ? AppColors.textSecondary : AppColors.white60;
  ...
  static const Color checkInProgressTrack =
      _isLight ? AppColors.divider : AppColors.white04;
  static const Color checkInProgressFill = Color(0xFFFD7E2F); // light-audit: keep-dark 进度橙，两态一致
  // 里程碑节点圆点：深色深底/深描边，浅色翻为浅灰点（避免浅底上黑块）。
  static const Color checkInProgressDotFill =
      _isLight ? AppColors.border : Color(0xFF1B212A);
  static const Color checkInProgressDotStroke =
      _isLight ? AppColors.divider : Color(0xFF121721);
  static const Color checkInMilestoneBubbleStart = Color(0xFFFFD443); // light-audit: keep-dark 里程碑气泡黄
  static const Color checkInMilestoneBubbleEnd = AppBrandColors.accent;
  // 里程碑数值 / 文字：深色白字，浅色墨字（浅底可读）。
  static const Color checkInMilestoneAmount =
      _isLight ? AppColors.textPrimary : AppColors.white100;
  static const Color checkInMilestoneLabel =
      _isLight ? AppColors.textSecondary : AppColors.white60;
  static const Color checkInCtaSolid = AppBrandColors.checkInYellow;
  // 签到 CTA 文字/spinner（坐在主强调色上）：深色黄底深字，浅色粉底翻白。
  static const Color checkInCtaTextDark =
      _isLight ? AppColors.onPrimary : AppBrandColors.textOnLightPanel;
  ...
  static const Color taskDivider =
      _isLight ? AppColors.divider : AppColors.white04;
  static const Color taskActionBg =
      _isLight ? AppColors.surfaceSoft : AppColors.white08;
  static const Color taskActionHighlightBg = AppBrandColors.checkInYellow;
  // 高亮任务按钮文字（坐在主强调色上）：深色黄底深字，浅色粉底翻白。
  static const Color taskActionHighlightText =
      _isLight ? AppColors.onPrimary : AppBrandColors.textOnLightPanel;
  static const Color taskRewardChipBg =
      _isLight ? AppColors.surfaceSoft : AppColors.white04;
  static const Color taskRewardChipMutedBg = AppColors.black04;
  static const Color taskRewardChipText =
      _isLight ? AppColors.textPrimary : AppColors.white100;
  static const Color taskRewardChipMutedText =
      _isLight ? AppColors.textSecondary : AppColors.white60;
  static const Color taskTimelineTrack =
      _isLight ? AppColors.divider : AppColors.white08;
  static const Color taskTimelineFill = Color(0xFFFD7E2F); // light-audit: keep-dark 进度橙
  static const Color taskTimelineBubbleActive = AppBrandColors.checkInHighlightHeader;
  static const Color taskTimelineBubbleReached =
      _isLight ? AppColors.surfaceSoft : AppColors.white04;
  static const Color taskTimelineDot =
      _isLight ? AppColors.border : Color(0xFF1C2129);
  static const Color taskTimelineDotReached = Color(0xFFFD7E2F); // light-audit: keep-dark 已达节点橙
  static const Color taskTimelineDotBorder =
      _isLight ? AppColors.divider : Color(0xFF131720);
  ...
  static const Color taskPopularGradientStart = Color(0xFFFF5065); // light-audit: keep-dark 热门角标渐变
  static const Color taskPopularGradientEnd = Color(0xFFFFB06B); // light-audit: keep-dark 热门角标渐变
  static const Color taskVipGradientBadgeText = AppBrandColors.vipOnGradientText;
  static const Color taskProgressLabel =
      _isLight ? AppColors.textSecondary : AppColors.white60;
  // VIP 每日能量条：左→右由半透明白渐隐为全透明（Figma 559:23239）。
  static const Color vipEntryBadgeGradientStart = Color(0x52FFFFFF); // light-audit: effect VIP 能量条白渐隐
