...
class CategoryMockDataSource implements CategoryDataSource {
  ...
  Future<CategoryPageContent> fetchPageContent() async {
    ...
    return CategoryPageContent(filterGroups: _filterGroups, items: _items);
  }
  ...
  static const List<CategoryFilterGroup> _filterGroups = [
    ...
    CategoryFilterGroup(id: 'status', options: ['全部', '完结', '连载', '3日内更新']),
    CategoryFilterGroup(id: 'type', options: ['全部', '互动故事', '自由成长']),
    CategoryFilterGroup(id: 'sort', options: ['综合', '按热度', '按新作', '按互动']),
  ];
