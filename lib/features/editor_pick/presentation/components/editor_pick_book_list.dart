...
/// L3 — 编辑推荐详情列表（builder 渲染的 sliver，行间 16px 间距）。
class EditorPickBookList extends StatelessWidget {
  const EditorPickBookList({super.key, required this.items, this.onItemTap});
    ...
    return SliverPadding(
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate((context, index) {
          final row = EditorPickBookRow(item: items[index], onTap: onItemTap);
